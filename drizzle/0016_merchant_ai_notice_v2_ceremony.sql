-- Merchant AI notice v2 and the consent ceremony.
--
-- merchant-ai-notice-2026-09-15.v1 lets one consent ceremony authorize AI for
-- several Properties at once and removes the password step-up from consent
-- (decisions 3 and 4, docs/plan/property-setup-exploration.md §6). Consent is
-- recorded per Property as before; this migration makes the new notice
-- recordable and names the ceremony each evidence row came from.
--
-- 1. Both contract CHECKs gain the (version, digest) arm for the new notice.
--    They are append-only OR-chains, so consent recorded under an earlier
--    notice stays valid at its own version.
-- 2. `merchant_ai_consent_evidence.ceremony_id` is the consent ceremony that
--    wrote the row: a fresh id for a single-Property command, one shared id for
--    every row of a multi-Property ceremony. Existing rows and restore resets,
--    which are not a merchant's consent, leave it null.
-- 3. `apply_merchant_ai_transition_v1` gains a trailing `p_ceremony_id uuid
--    DEFAULT NULL`. PostgreSQL cannot add a parameter in place, and a second
--    overload would make every 26-argument call ambiguous, so the function is
--    dropped and recreated with the 0006 body plus the new column. Existing
--    26-argument callers keep working and record a null ceremony.
ALTER TABLE "merchant_ai_consent_evidence" ADD COLUMN "ceremony_id" uuid;--> statement-breakpoint
...
ALTER TABLE "merchant_ai_consent_evidence" ADD CONSTRAINT "merchant_ai_consent_evidence_contract_valid" CHECK ((
          ...
          OR ("merchant_ai_consent_evidence"."notice_version" = 'merchant-ai-notice-2026-09-15.v1'
            AND "merchant_ai_consent_evidence"."notice_digest" = '6c98ae3bb57b5b142afed1749a1b9f59be8d6ca7edaca3250e39055897289a9c')
        )
...
ALTER TABLE "merchant_ai_enablement" ADD CONSTRAINT "merchant_ai_enablement_contract_valid" CHECK ((
          ...
          OR ("merchant_ai_enablement"."notice_version" = 'merchant-ai-notice-2026-09-15.v1'
            AND "merchant_ai_enablement"."notice_digest" = '6c98ae3bb57b5b142afed1749a1b9f59be8d6ca7edaca3250e39055897289a9c')
        )
...
BEGIN
  ...
  IF p_transition_kind IN ('enable', 'change') THEN
    reset_analysis_watermark :=
      'review_analysis' = ANY(p_capabilities)
      AND expected_review_analysis_epoch
        > COALESCE(current_head.review_analysis_epoch, 0);
    IF reset_analysis_watermark THEN
      ...
      END IF;
    ELSIF p_analysis_start_sequence
      <> COALESCE(current_head.analysis_start_sequence, 0)
    THEN
      RAISE EXCEPTION 'merchant_ai_invalid_analysis_sequence' USING ERRCODE = 'P0001';
    END IF;
  END IF;
  ...
  INSERT INTO merchant_ai_consent_evidence (
    ...
    p_idempotency_key, p_request_hash, p_occurred_at, p_ceremony_id
  )
