-- Repair the AI property admission authority. Two independent defects in
-- `admit_ai_property_v1` (introduced in 0049) made it impossible for ANY property
-- to be admitted, which is why no `ai_review_analyses` row has ever existed in the
-- closed beta while the synthetic canary passed (the canary carries no property and
-- takes a different branch).
--
-- 1. Capability vocabulary. `organization_capability.capability` holds PURPOSES
--    (`ai.analyze`, `ai.generate_reply`, `ai.detect_trends` — see
--    CAPABILITY_BY_PURPOSE in merchant-ai-authorization.repository.ts). The guard
--    compared it against `capability_name` (`review_analysis`, `reply_drafting`,
--    `property_trends`), so the row could never be found. `operation_profile_row`
--    is already in scope and carries the purpose.
-- 2. Suspension polarity. `property_policy` rows exist only where
--    setPropertyPolicy has run (the ops suspend/restore command). Requiring a row
--    with `suspended_at IS NULL` denied every property that has never been
--    suspended — in this database that is every property except one.
--
-- Both are corrected below; nothing else in the function body changes.
