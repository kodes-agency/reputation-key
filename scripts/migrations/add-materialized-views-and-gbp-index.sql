...
-- Computed from inbox_items. Columns: org, property, date, new/addressed counts, avg response hours.
...
SELECT
  ...
  count(*) FILTER (WHERE status = 'addressed')::integer AS addressed_count,
  avg(EXTRACT(EPOCH FROM (addressed_at - created_at)) / 3600)::real AS avg_response_hours
FROM inbox_items
