// Which Google the local stack talks to.
//
// `pnpm local:up` defaults to the Compose sandbox: `e2e/stack.env` selects
// `GOOGLE_PROVIDER_ENDPOINT_PROFILE=local-sandbox` and pins every GBP/OAuth
// base URL at `provider-sandbox:4100`. `REPKEY_LOCAL_GOOGLE=real` in the
// gitignored `local.env` removes those pins so the app resolves Google's own
// approved endpoints (`composition/provider-runtime.ts`
// GOOGLE_PROVIDER_ENDPOINTS) and runs the real OAuth handshake, the real
// Account Management + Business Information imports, and the real review sync.
//
// Nothing about the real path is production-only: outside `NODE_ENV=production`
// the four provider keyrings derive local fallbacks from `OAUTH_STATE_SECRET`
// and the opaque OAuth state lives in an in-memory provider-ephemeral store
