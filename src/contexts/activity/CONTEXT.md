...
- Activity contributes to the Identity-orchestrated Organization Export
  ...
  whose actor is redacted exports `Former member`/`system` even when the bounded
  rewrite has not reached that row yet — and the fence itself is never exported.
  An Organization with no retained feed rows contributes the affirmative
  `no_data`; it never falls back to the restricted history.
