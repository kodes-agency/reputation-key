// Feed activity surface — server functions
    ...
    tracedHandler(
      async ({ data }) => {
        ...
        try {
          ...
          return feedPublicApi.getActivityTimeline(
            {
  ...
  .handler(
    tracedHandler(
      async ({ data }) => {
        ...
        try {
          const { feedPublicApi } = getContainer()
          return feedPublicApi.listRecentActivity(
            {
  ...
  .handler(
    tracedHandler(
      async ({ data }) => {
        ...
        try {
          return getContainer().feedPublicApi.listOperationalActionHistory(
  ...
  .handler(
    tracedHandler(
      async ({ data }) => {
        ...
        await requireExecutionAllowed({ actor: ctx, action: 'policy.admin' })
        try {
          return getContainer().feedPublicApi.exportOperationalActionHistory(
            operationalHistoryInput(data),
            ctx,
          )
        } catch (error) {
