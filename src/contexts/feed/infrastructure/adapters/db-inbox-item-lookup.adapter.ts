// Feed activity surface — DB-backed inbox item lookup adapter
// Maps source context IDs (reviewId, feedbackId) → inboxItemId.
// Used by the durable reply-fact consumer so reply activity entries appear in
// the inbox item timeline.
...
import { activityError } from '../../domain/activity-errors'
...
export const createDbInboxItemLookupAdapter = (db: Database): InboxItemLookupPort => ({
  findBySourceId: async (sourceId, orgId): Promise<string | null> => {
    try {
      ...
      // this source". The durable consumer propagates this failure so delivery
      // retries rather than writing a silently mis-linked activity row. The
      // not-found case (empty result) still returns null above; only a thrown DB
      // error reaches here.
