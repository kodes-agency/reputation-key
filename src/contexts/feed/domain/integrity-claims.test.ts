...
describe('Recent Activity integrity-claim boundary', () => {
  it('contains no row-hash implementation or cryptographic integrity claim', () => {
    const offenders = walk(join(ROOT, 'src/contexts/feed'))
  ...
  it('does not describe the product projection as an audit log or audit trail', () => {
    const vocabularyFiles = [
      ...walk(join(ROOT, 'src/contexts/feed')),
  ...
  it('does not expose deprecated ActivityLog domain aliases', () => {
    const files = [
      join(ROOT, 'src/contexts/feed/domain/activity-types.ts'),
      join(ROOT, 'src/contexts/feed/domain/activity-constructors.ts'),
      join(ROOT, 'src/contexts/feed/application/public-api.ts'),
    ]
