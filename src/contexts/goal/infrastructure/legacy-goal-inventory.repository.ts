...
import {
  ...
  type LegacyGoalForeignKey,
  type LegacyGoalInventoryReport,
  type LegacyGoalReferentialAction,
  type LegacyGoalTableName,
} from '../application/legacy-goal-inventory'

type Row = Readonly<Record<string, unknown>>

const TABLE_NAMES = LEGACY_GOAL_TABLES.map(({ tableName }) => tableName)
for (const tableName of TABLE_NAMES) {
  if (!/^[a-z][a-z0-9_]*$/.test(tableName)) {
    throw new Error('legacy_goal_inventory_table_name_invalid')
  }
}
...
function tableCount(row: Row): Readonly<{
  tableName: LegacyGoalTableName
  rowCount: number
}> {
  const tableName = String(row.table_name)
  const countText = String(row.row_count)
  if (
    !TABLE_NAMES.includes(tableName as LegacyGoalTableName) ||
  ...
  return { tableName: tableName as LegacyGoalTableName, rowCount }
}

const REFERENTIAL_ACTIONS: Readonly<
  Record<'a' | 'r' | 'c' | 'n' | 'd', LegacyGoalReferentialAction>
> = Object.freeze({
  a: 'no_action',
  r: 'restrict',
  c: 'cascade',
...
function foreignKey(row: Row): LegacyGoalForeignKey {
...
export const readLegacyGoalInventory = async (
  db: Database,
  evaluatedAt: Date,
): Promise<LegacyGoalInventoryReport> =>
  db.transaction(
    async (snapshot) => {
      ...
      return buildLegacyGoalInventoryReport({
        evaluatedAt,
