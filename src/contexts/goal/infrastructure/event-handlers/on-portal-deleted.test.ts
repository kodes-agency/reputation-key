...
function makeFakeDeps(storedGoals: Goal[] = []) {
    ...
    list: async (filter) => {
      return storedGoals.filter((g) => {
        ...
        if (filter.organizationId && g.organizationId !== filter.organizationId)
          return false
        if (filter.portalId && g.portalId !== filter.portalId) return false
        return true
      })
    },
    listInstances: async () => [],
  ...
  type SystemCancelFn = OnPortalDeletedDeps['systemCancelGoalFn']
...
describe('onPortalDeleted', () => {
  it('cancels active goals scoped to the deleted portal', async () => {
    const g1 = makeGoal({
      id: goalId('g-1'),
      portalId: portalId('portal-1'),
      status: 'active',
  ...
  it('does not cancel goals scoped to other portals', async () => {
  ...
  it('passes the portal_deleted reason tag to systemCancelGoalFn', async () => {
    const g1 = makeGoal({
      id: goalId('g-1'),
      portalId: portalId('portal-1'),
      status: 'active',
    ...
    expect(fakes.systemCancelGoalFn).toHaveBeenCalledWith(
      expect.objectContaining({ reason: 'portal_deleted' }),
    )
  ...
  it('logs error when systemCancelGoalFn throws (not returns Err)', async () => {
    ...
    const fakes = makeFakeDeps([g1])
    const handler = onPortalDeleted({ ...fakes.deps, systemCancelGoalFn: throwingCancel })
