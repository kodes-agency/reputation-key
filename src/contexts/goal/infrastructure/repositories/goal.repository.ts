...
function safeMapGoals(
  rows: ReadonlyArray<typeof goals.$inferSelect>,
  ...
  return rows.flatMap((row) => {
    try {
      ...
      logger.warn({ err: e }, 'Skipping goal with invalid DB data')
      return []
    }
  })
}
...
export const createGoalRepository = (
  ...
  const { clock, monotonicNow } = runtime
  const log = runtime.logger.child({ component: 'goal-repo' })
  ...
  return {
    ...
    insert: async (goal) => {
      return trace('goal.insert', async () => {
        ...
        log.debug({ duration: monotonicNow() - start }, 'goal insert complete')
    ...
    list: async (filter: GoalListFilter) => {
      return trace('goal.list', async () => {
        ...
        log.debug(
          { count: rows.length, duration: monotonicNow() - start },
          'goal list complete',
        )
        return safeMapGoals(rows, log)
      })
    },
    ...
    listInstances: async (parentGoalId, orgId) => {
      return trace('goal.listInstances', async () => {
        const rows = await db
          ...
          .from(goals)
          .where(
            and(eq(goals.parentGoalId, parentGoalId), eq(goals.organizationId, orgId)),
          )
        return safeMapGoals(rows, log)
      })
    },
    ...
    findActiveGoalsByMetric: async (
      ...
      return trace('goal.findActiveGoalsByMetric', async () => {
        ...
        const rows = await db
          ...
          .where(and(...conditions))
        log.debug(
          { metricKey, count: rows.length, duration: monotonicNow() - start },
