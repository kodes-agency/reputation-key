...
import { verifyOneClickUnsubscribeToken } from '../application/one-click-unsubscribe-token'
...
export async function handleOneClickUnsubscribePost(request: Request): Promise<Response> {
  return trace('notification.oneClickUnsubscribe', async () => {
    const logger = getLogger()
    const rawKeys = getEnv().NOTIFICATION_UNSUBSCRIBE_HMAC_KEYS
    if (!rawKeys) {
    ...
    try {
      ...
      const scopes =
        await getContainer().notificationPublicApi.oneClickUnsubscribe(target)
      logger.info(
