...
| **Cycle Head**                | The compare-and-swap pointer to the one current source cycle, its current source fence, workflow status, and state revision. An unchanged Review source-epoch carry may advance the fence while retaining the same cycle and status.                                                |
...
- Feedback outcome history is insert-only through the handling store. The optional internal note is returned only when the caller has current `inbox.write ∧ feedback.handle` Property scope; it is absent from guest surfaces and recorded Inbox lifecycle facts.
...
- A Review cycle opening is insert-only through the Review handling store. Starting a later cycle advances the head only when expected cycle number, Material Review Revision, and state revision all match.
