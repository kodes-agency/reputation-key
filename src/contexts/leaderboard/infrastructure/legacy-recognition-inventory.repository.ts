...
import {
  ...
  type LegacyRecognitionForeignKey,
  type LegacyRecognitionInventoryReport,
  type LegacyRecognitionTableName,
} from '../application/legacy-recognition-inventory'

type Row = Readonly<Record<string, unknown>>

const TABLE_NAMES = LEGACY_RECOGNITION_TABLES.map(({ tableName }) => tableName)
for (const tableName of TABLE_NAMES) {
  if (!/^[a-z][a-z0-9_]*$/.test(tableName)) {
    throw new Error('legacy_recognition_inventory_table_name_invalid')
  }
}
...
function tableCount(row: Row): Readonly<{
  ...
  if (
    !TABLE_NAMES.includes(tableName as LegacyRecognitionTableName) ||
  ...
  return { tableName: tableName as LegacyRecognitionTableName, rowCount }
}
...
function foreignKey(row: Row): LegacyRecognitionForeignKey {
...
export async function readLegacyRecognitionInventory(
  db: Database,
  evaluatedAt: Date,
): Promise<LegacyRecognitionInventoryReport> {
  return db.transaction(
    async (snapshot) => {
      ...
      return buildLegacyRecognitionInventoryReport({
        evaluatedAt,
