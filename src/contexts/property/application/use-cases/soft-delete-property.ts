...
export const deleteProperty =
  ...
  async (_input: DeletePropertyInput, _ctx: AuthContext): Promise<void> => {
    // LIF-01 containment. Ordinary Property lifecycle uses the separate,
    // recoverable Archive/Restore/Property-binding-disconnect commands.
    // Permanent erasure remains a distinct support-mediated operation with
    // its own verification and evidence, so this legacy product use case has
    // no destructive dependencies and always refuses before any effect.
