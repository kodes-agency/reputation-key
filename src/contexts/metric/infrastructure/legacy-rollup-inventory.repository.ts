...
import {
  ...
  type LegacyRollupForeignKey,
  type LegacyRollupInventoryReport,
  type LegacyRollupReferentialAction,
  type LegacyRollupTableName,
} from '../application/legacy-rollup-inventory'

type Row = Readonly<Record<string, unknown>>

const TABLE_NAMES = LEGACY_ROLLUP_TABLES.map(({ tableName }) => tableName)
for (const tableName of TABLE_NAMES) {
  // `_rollup_watermarks` starts with an underscore, so the pattern allows a
  // leading underscore while still refusing anything that could be quoted out
  // of the generated statement.
  if (!/^_?[a-z][a-z0-9_]*$/.test(tableName)) {
    throw new Error('legacy_rollup_inventory_table_name_invalid')
  }
}
...
function tableCount(row: Row): Readonly<{
  tableName: LegacyRollupTableName
  rowCount: number
}> {
  const tableName = String(row.table_name)
  const countText = String(row.row_count)
  if (
    !TABLE_NAMES.includes(tableName as LegacyRollupTableName) ||
  ...
  return { tableName: tableName as LegacyRollupTableName, rowCount }
}

const REFERENTIAL_ACTIONS: Readonly<
  Record<'a' | 'r' | 'c' | 'n' | 'd', LegacyRollupReferentialAction>
> = Object.freeze({
  a: 'no_action',
  r: 'restrict',
  c: 'cascade',
...
function foreignKey(row: Row): LegacyRollupForeignKey {
 ...
 * metadata describe the same instant. A read-write transaction here could not
 * be distinguished from a contraction attempt in an audit log.
 */
export const readLegacyRollupInventory = async (
  db: Database,
  evaluatedAt: Date,
): Promise<LegacyRollupInventoryReport> =>
  db.transaction(
    async (snapshot) => {
      ...
      return buildLegacyRollupInventoryReport({
        evaluatedAt,
