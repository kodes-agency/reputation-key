...
- Rollback never rewrites history: it closes the current activation and appends a new activation to an older valid snapshot. Portal command stores expose no snapshot mutation and constrain activation updates to a one-time interval closure; hard deletion exists only in the irreversible Organization lifecycle purge. Disable/archive closes the activation, and token revocation always prevents resolution.
...
- Core Portal creation, update/publication transition, and soft deletion use one Portal-owned command store. Authoritative Portal state, initial responsibility, live-token revocation on delete, and every required lifecycle outbox fact commit in the same PostgreSQL transaction.
...
- **`completeContentReview`** — Lock the Portal first, then read its child configuration in a new statement so a lock wait cannot pair a new Portal revision with a stale statement snapshot. Record the completed review and its three derived workflow facts at one DB-returned Portal revision. Replay is recognized by the locked Organization/Property/Portal/Review/revision identity, so it neither duplicates facts nor advances the revision; event IDs remain constructor-generated facts rather than caller-owned idempotency keys.
