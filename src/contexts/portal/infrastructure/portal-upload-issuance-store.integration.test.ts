...
describe.sequential('Portal upload issuance store (real PostgreSQL)', () => {
  ...
  it('prevents a stale worker from replacing the newer upload derivative', async () => {
    ...
    const portal = await getPool().query(
      'SELECT hero_image_url FROM portals WHERE id = $1 AND organization_id = $2',
      [PORTAL_A, ORG_A],
    )
    const rows = await getPool().query(
      `SELECT id, state FROM portal_upload_issuances
    ...
    expect(rows.rows).toEqual([
      { id: UPLOAD_A, state: 'superseded' },
      { id: UPLOAD_B, state: 'finalized' },
    ])
  })
})
