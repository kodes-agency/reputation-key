...
const createInMemoryPortalGroupRepo = (): PortalGroupRepository & {
  ...
  return {
    ...
    insert: async (orgId, group) => {
      if (String(group.organizationId) !== String(orgId)) throw new Error('Tenant mismatch')
    ...
    findPortalMembership: async (_orgId, pid) =>
      memberships.get(String(pid)) ?? null,
...
const setup = () => {
  ...
  const deps = {
    ...
    propertyApi: {
      ...
      importProperty: async () => { throw new Error('not implemented') },
