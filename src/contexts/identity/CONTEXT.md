...
- **Membership Authority** — Better Auth `member` rows are the sole Organization-membership authority. Beta requires exactly one distinct Organization per interactive login.
...
- User → Organization (exactly one Better Auth membership in beta; conflicting legacy memberships require support resolution).
...
- Invitation acceptance atomically locks and rechecks the user's Better Auth memberships; an existing membership in another Organization cannot create a second one.
...
- Registration requires the exact pending, unexpired, email-bound manager invitation. Before Better Auth runs, the app stores a content-free recovery payload with preallocated user/account/session IDs in Better Auth's `verification` table. Recovery may resume exact provider authority, compensate only those recorded IDs, or stop in `manual_review`; it never deletes by email or inferred ownership.
...
- An Organization closure request is authorized from a transactionally locked exact `owner` membership. It atomically commits lifecycle revision, 30-day recovery deadline, Organization suspension/policy generation, retry receipt, and a content-minimal durable fact.
...
- Removing a member transactionally revokes all of that user's Better Auth sessions, deletes the membership, and records the durable removal fact. Cross-context manager/provider authorities are fenced before this commit; the final AccountAdmin cannot be removed.
...
`identity.member.invited` is identifier-only (v2 is the only durable shape): the
invitee email remains in the canonical invitation row and is absent from the
domain event; the outbox CHECK rejects any `email` key on the payload.
...
- `organization.update` — Update beta organization identity settings (name, slug, logo, contact)
