...
describe('advance review-analysis backfill — durable membership', () => {
  it.each([
    {
      label: 'emits the exact pinned Material Review Revision',
      pinnedSourceRevision: 7,
      expectedOutcome: 'emitted',
      expectedEmitted: [THIRD_REVIEW_ID],
      expectedSkips: 0,
    },
    {
      label: 'skips instead of substituting a newer Material Review Revision',
      pinnedSourceRevision: 6,
    ...
    '$label',
    async ({ pinnedSourceRevision, expectedOutcome, expectedEmitted, expectedSkips }) => {
      ...
      const session: ReviewAnalysisBackfillSession = {
        ...
        readActiveRun: async () => ({
          ...run,
          skippedReviewCount: run.skippedReviewCount + skips,
        }),
        openRun: async () => {
          throw new Error('not used')
        },
        readRunMember: async (input) => {
          ...
          return input.ordinal === 2
            ? { reviewId: THIRD_REVIEW_ID, sourceRevision: pinnedSourceRevision }
            : null
        },
      ...
      expect(skips).toBe(expectedSkips)
    },
  )
})
