...
- **review.on-property-archived** — Durable worker consumer that cancels every active publication (`requested`/`authorized`/`sending`/`pending_observation`) of an archived Property's reviews, exactly as a Google disconnect does: the Reply returns to draft and one `review.reply.publication_cancelled` fact (cause `policy`) is recorded per Reply, so the provider authorizer's refusal is never reported to the author as a Google rejection. A fact delivered after a Restore finds the Property active and cancels nothing.
...
- **Publication scope** — Approve, edit-and-republish and **Try publishing again** refuse before any write, in words, when the Review's Property is not active ("This property has been removed…") or the Review has not been observed at the Property's current source epoch since a Restore or relink ("…Try again after the next sync."). The provider authorizer would refuse either write, and the worker would report the refusal as a Google rejection.
