...
- **Material Review Revision** — Numbered exact binding for original rating and normalized original guest text. Rating or normalized-text changes create business revisions; an unchanged Review carried into a newer source epoch also receives a new exact-binding revision so historical foreign-key targets remain immutable. Provider metadata, translation, profile-photo, and observation-time changes stay on the current revision within one epoch.
...
- Material comparison uses `review-material-v1`: NFC-normalize the original guest text, collapse Unicode whitespace to one ASCII space, trim, and represent an empty result as null. Case and punctuation remain significant. Only rating or this normalized text creates a business-material change; an epoch carry still advances `sourceRevision` to preserve immutable exact bindings and is explicitly attested as non-material to Inbox.
