...
import type { PropertyEventPort, PropertyCreatedEvent } from '../../application/ports/property-event.port'
import type { EventBus } from '#/shared/events/event-bus'
import { propertyCreated } from '#/contexts/property/application/public-api'
import { propertyId, organizationId as toOrgId, googleConnectionId as toGoogleConnectionId } from '#/shared/domain/ids'
  ...
  emitPropertyCreated: async (event: PropertyCreatedEvent) => {
    await eventBus.emit(
      propertyCreated({
        ...
        googleConnectionId: event.googleConnectionId ? toGoogleConnectionId(event.googleConnectionId) : undefined,
