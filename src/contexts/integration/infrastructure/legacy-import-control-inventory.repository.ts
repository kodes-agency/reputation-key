...
import {
  ...
  type LegacyImportControlForeignKey,
  type LegacyImportControlInventoryReport,
  type LegacyImportControlReferentialAction,
  type LegacyImportControlTableName,
} from '../application/legacy-import-control-inventory'

type Row = Readonly<Record<string, unknown>>

const TABLE_NAMES = LEGACY_IMPORT_CONTROL_TABLES.map(({ tableName }) => tableName)
for (const tableName of TABLE_NAMES) {
  if (!/^[a-z][a-z0-9_]*$/.test(tableName)) {
    throw new Error('legacy_import_control_inventory_table_name_invalid')
  }
}
...
function tableCount(row: Row): Readonly<{
  tableName: LegacyImportControlTableName
  rowCount: number
}> {
  const tableName = String(row.table_name)
  const countText = String(row.row_count)
  if (
    !TABLE_NAMES.includes(tableName as LegacyImportControlTableName) ||
  ...
  return { tableName: tableName as LegacyImportControlTableName, rowCount }
}

const REFERENTIAL_ACTIONS: Readonly<
  Record<'a' | 'r' | 'c' | 'n' | 'd', LegacyImportControlReferentialAction>
> = Object.freeze({
  a: 'no_action',
  r: 'restrict',
  c: 'cascade',
...
function foreignKey(row: Row): LegacyImportControlForeignKey {
 ...
 */
export const readLegacyImportControlInventory = async (
  db: Database,
  evaluatedAt: Date,
): Promise<LegacyImportControlInventoryReport> =>
  db.transaction(
    async (snapshot) => {
      ...
      return buildLegacyImportControlInventoryReport({
        evaluatedAt,
