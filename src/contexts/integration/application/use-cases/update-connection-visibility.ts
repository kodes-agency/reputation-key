...
export const updateConnectionVisibility =
  ...
  async (input: UpdateConnectionVisibilityInput, ctx: AuthContext): Promise<GoogleConnection> => {
    // 1. Authorize
    ...
    // 3. Update visibility
    await deps.connectionRepo.updateVisibility(connectionId, input.visibility)

    const updatedConnection = await deps.connectionRepo.findById(ctx.organizationId, connectionId)
