...
//
// Two cheap aggregate reads, gathered once per health-check run (5-min
...
export function createAlertAuxReader(deps: AlertAuxReaderDeps): AlertAuxReader {
  return {
    read: async () => {
      const [retentionFailedSubjects, betaFeedbackTriage] = await Promise.all([
      ...
      return { retentionFailedSubjects, betaFeedbackTriage }
    },
  }
}
