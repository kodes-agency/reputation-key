 ...
 * Written once, in the transaction that starts or joins the epoch's first
 * import run, and read for every later provider observation: a Review first
 * seen in the epoch whose Google publication time is at or before the cutoff
 * is onboarding history, whichever run observes it. Snapshot runs are swept
 * 30 days after they end, so the cutoff cannot live on the run.
 */
