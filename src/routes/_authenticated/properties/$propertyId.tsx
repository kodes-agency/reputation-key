...
function PropertyLayout() {
	...
	async function handleDelete() {
		if (!window.confirm("Are you sure you want to delete this property?"))
			return;
		await deleteMutation({ data: { propertyId } });
	}
				...
				<CardContent>
					<Tabs value={activeTab} className="w-full">
						<TabsList>
							...
							<TabsTrigger value="teams" asChild>
								<Link
									to="/properties/$propertyId/teams"
									params={{ propertyId }}
								>
									Teams
								</Link>
					...
					<div>
						...
						<p className="mb-3 text-sm text-muted-foreground">
							This property will be hidden from your organization. Its data will
							be preserved but it will no longer appear in lists.
						</p>
						<Button
							variant="outline"
