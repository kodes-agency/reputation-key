...
import { useMutationAction, useMutationActionSilent } from '#/components/hooks/use-mutation-action'
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
...
function PortalLinksPage() {
  ...
  const handleReorderLinks = async (categoryId: string, reordered: LinkItem[]) => {
    ...
    setLinks([...otherLinks, ...reordered.map((l, i) => ({ ...l, sortKey: updates[i].sortKey }))])
  ...
  return (
    <div className="page-wrap px-4 pb-8 pt-14">
      ...
      <PortalTabNav propertyId={propertyId} portalId={portalId} activeTab="links" />
      ...
      <Card className="island-shell rise-in rounded-2xl">
        ...
        <CardContent>
          {canEdit && <CategoryAddForm onSubmit={handleAddCategory} />}
