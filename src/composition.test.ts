// BQC-6.5 — applyProviderEndpointOverrides: the composition root's single seam
// between the approved Google provider endpoints and the operator sandbox.
//
// Fail closed (ADR 0048/0031): a deployed cell never honours an endpoint
// override; the local Compose stack is the only legal sandbox target.
...
describe('applyProviderEndpointOverrides (BQC-6.5 operator sandbox seam)', () => {
  const approved = GOOGLE_PROVIDER_ENDPOINTS
