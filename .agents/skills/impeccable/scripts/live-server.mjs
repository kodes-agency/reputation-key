...
async function pushApplyBatchInChunksAndWait(batch, pageUrl, context = {}) {
  ...
  return {
    status:
      failed.length === 0 ? 'done' : appliedEntryIds.length > 0 ? 'partial' : 'error',
    appliedEntryIds,
    failed,
    files: [...files],
    notes,
...
function validateManualApplyResultMessage(msg, deferred) {
  ...
  for (const entryId of data.appliedEntryIds) {
    if (eventEntryIds.size > 0 && !eventEntryIds.has(entryId)) {
      return invalidManualApplyResult('applied_entry_id_not_in_event', eventId, {
        entryId,
      })
    }
  }
    ...
    if (p === '/manual-edit-commit' && req.method === 'POST') {
      ...
      ;(async () => {
        ...
        try {
          ...
          const requestedMode = (process.env.IMPECCABLE_LIVE_COPY_AGENT || 'auto')
            .trim()
            .toLowerCase()
          const useChatRoute =
            requestedMode === 'chat' ||
    ...
    if (p === '/manual-edit-repair-decision' && req.method === 'POST') {
      ...
      req.on('end', () => {
        ...
        if (action !== 'rollback') {
          res.writeHead(400, { 'Content-Type': 'application/json' })
          res.end(
            JSON.stringify({ error: 'unsupported_manual_edit_repair_decision', action }),
          )
          return
        }
