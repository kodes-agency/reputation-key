...
- **GAP — the merchant AI notice is false as shipped.** The digest-pinned notice promises prompt caching "at most one hour" (`merchant-ai-notice-contract.ts:93`); every request sends `prompt_cache_retention: '24h'` (`ai-openai-request-contract.ts:21,211`) — 24× the stated maximum. Forced by `gpt-5.6-luna` rejecting `in_memory` with HTTP 400; the model-switch document required sign-off _before_ landing precisely because it changes a merchant-facing retention claim (`docs/operations/model-switch-5-6-luna-2026-08-19.md:419-420`). Never signed off, notice text and digest never updated, so every beta merchant consented to wording the system does not honour. No duration was promised to Google, but "materially longer provider content retention" is a re-submission trigger in their change-control rule. **Decision owed:** correct the notice text, bump `noticeVersion` and its digest, re-collect merchant consent, and decide whether Google needs re-notification.
...
- **GAP — the privacy notice merchants are sent to cannot be published.** The AI consent dialog links `/privacy` and `/privacy#contact`; neither is an application route, and the backing document is not `approved`, so the repo's own `isLegalLinkPublishable('/privacy')` returns `false` (`legal-link-targets.ts:149-162,204-213`). **Decision owed:** approve and publish the notice with a real route, or stop linking it from a consent dialog.
