...
**Google disposition update:** The [written support response](google-business-profile-ai-policy-response-2026-07-14.md) conditionally permits the submitted per-property AI architecture. Any pre-response “unresolved Google policy” finding below is superseded by the requirement to implement ADR 0031 and its controls.
...
| **dashboard**    | Limited beta                                         | Raw review/metric queries remain; cache is unused; three materialized views are refreshed but never read. Google now permits independently generated per-property derived analysis, but the current read models do not yet prove raw/derived separation or policy versioning.                                                                                                | Bounded property-scoped reads, query budgets, ADR-0031 source capabilities, raw/derived lifecycle separation, and no cross-property or review-solicitation aggregate.                                                                                                           |
...
- accepted ADR 0031 and executable Google-policy controls before Google-derived AI or aggregate features.
...
Google's written response permits per-property sentiment, scores, categories, themes, trends, summaries, external AI processing, and manager-reviewed drafts. None should be enabled until the response is converted into ADR 0031 and release capabilities enforcing raw-content refresh/removal, separate derived metadata, PII removal, no provider training/minimum retention, property-region routing, merchant opt-in, and manual publication. Historical backfill and durable few-shot replies retain the conservative caveats in the disposition because they were not answered individually. Review-solicitation gamification remains prohibited independently.
...
Proceed with the beta-readiness program in [the beta master plan](beta-readiness-master-plan.md), absorbing PRE17A/B/C wherever the same reliability, review lifecycle, regional routing, observability, or scale work is already planned. Phase 17/18 planning is now externally unblocked, but implementation should wait for the user's signal and must not release until the beta core and PRE17's explicit AI/policy controls are satisfied.
