...
| Baseline backlog | 7 unused files · 104 unused exports · 121 unused types · 6 unused deps · 5 unused devDeps · 3 test-only deps · 1 duplicate export. **Repo is NOT clean** → use `gate: "new-only"` so the gate fails only on *new* issues, not the legacy 247. |
...
Semantics: `audit` defaults to `gate: "new-only"` — an agent editing a file with pre-existing dead exports is punished only for exports *it* adds. Runtime errors fail **open** (one stderr line) so a missing binary never wedges the loop. `git push --no-verify` does **not** bypass it (it runs before the shell command).
