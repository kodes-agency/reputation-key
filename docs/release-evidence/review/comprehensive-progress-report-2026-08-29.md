...
**reverted**: the generated local-stack topology guard then pinned that
service's networks exactly, and the TLS session to the shared ingress closed on
connect besides. That rehearsal stack has since been retired; this paragraph
records the historical production-isolation finding.
...
2. a TCP relay in front of it on `google-admission-redis`, so the admission's
   pinned network list did not change;
3. an ACL user, a keypair and a `dnsName` for the relay generated by the
   then-current stack controller;
