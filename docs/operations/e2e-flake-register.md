...
| 2026-09-04 | `storybook-test` — `Integration/GoogleImport/ReviewForm › DraftSurvivesBackNavigation › play-test` | `thrown: "Exceeded timeout of 15000 ms for a test.`; passed on rerun of the same SHA (also seen 2026-09-03 on `StartFailure › smoke-test`) | 2                                                           | **Fixed by budget, cause measured.** In isolation the whole file runs in 7.4-8.8s and this story's play function takes 2352 ms (`pnpm test-storybook google-import-review-form --verbose`, 2026-09-04). CI runs ONE Storybook dev server against as many jest workers as the runner has cores, so the 15s jest default was a contention budget, not a hang budget. `pnpm test-storybook` now passes `--testTimeout 45000` (rationale in `.storybook/test-runner.ts`); a real hang is still bounded by each `findBy*`/`waitFor` inside the play function. Lowering `--maxWorkers` was measured on 2026-09-03 and rejected: 5m38s at 9 workers vs 5m18s baseline.                                                                                                                                                                                                                                 |
