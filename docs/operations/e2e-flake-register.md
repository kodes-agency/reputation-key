...
| 2026-09-04 | `storybook-test` — `Integration/GoogleImport/ReviewForm › DraftSurvivesBackNavigation › play-test` (and `StartFailure › smoke-test` on 2026-09-03) | `thrown: "Exceeded timeout of 15000 ms for a test.`; passed on rerun of the same SHA                                     | 2                                                           | **Fixed by bounding concurrency, cause measured.** Not the story: in isolation that whole file runs in 7.4–8.8s and the heaviest play function takes 2352 ms (`pnpm test-storybook google-import-review-form --verbose`, 2026-09-04). One Storybook **dev server** serves every jest worker and compiles a story's module graph on first visit, so unbounded workers race that compiler. Raising the per-test budget to 45s only moved the failure: 6/95 suites then failed inside `setupPage` with `page.goto: Timeout 30000ms exceeded` (a runner constant) while 529/529 tests passed. CI now runs `--maxWorkers 2 --testTimeout 30000`. Serving a static `storybook build` instead was tried and rejected: 74/95 suites fail because each play function runs before the built story's lazy chunk has mounted (the root is still empty at ~20 ms; the same story renders correctly when probed directly). |
