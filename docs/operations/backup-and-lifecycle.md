...
ops:restore-verify` — hard-requires `RESTORE_MODE=isolated` + an isolated
   target, runs the source-policy purge IN-PROCESS (the purge job's execution
   path, not BullMQ), writes the normal `retention_runs` evidence
   (`reviews.purge`), asserts ZERO expired-content rows remain eligible, and
   prints the evidence + cutover checklist. A restored environment must never
   serve expired source content.
...
(BQC-7.8) + `policy_decision_audit` / `audit_logs` at 365d, + `quarantine.ttl`,
`retention_runs` documented indefinite-by-design.
