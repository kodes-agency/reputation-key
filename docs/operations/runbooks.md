...
**What `notification.missing-for-inbox-item` means:** an inbox item past the grace edge still has no notification attached and its delivery is not decided — a review arrived, was projected into the inbox, and nobody has been told yet. Delivery is decided once the Feed consumer has taken the item's `inbox.inbox_item.created` fact and every delivery it made has settled; an item whose recipients all muted it, or no longer qualify, is decided and never counted. A non-zero count means the consumer has not taken the fact yet (outbox dispatch lag, repaired by dispatcher retries and published-event redelivery) or a delivery Redis accepted has not settled (repaired by `reconcile-missing-notifications`). A single occurrence pages because the manager-facing journey is already incomplete. Reviews an import brought in as Google history (`historical_onboarding`) are never announced and never counted, so an import's past reviews having no notification is expected (ADR 0046).
