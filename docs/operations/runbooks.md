...
**Alert:** `observability.snapshot-degraded` (P1) — at least one operations-snapshot section that an alert reads failed, outlasted its budget (5 seconds for a section, half that for one database signal), or was never reached on this evaluation. The detail names the degraded `health.<signal>` markers and every alert they blind.
