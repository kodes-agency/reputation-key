...
2. Worker logs for `[operations-snapshot] health signals degraded` — each degraded signal with its outcome and the section time elapsed: `failed` (the read rejected; `Span health.check.<signal> failed` carries its content-free error name), `timed_out` (it outlasted its own cap), or `not_started` (the section budget was spent before its turn — collateral, not a cause). The earliest `failed` or `timed_out` signal is the cause. If the first marked signal started with little budget left, or every marker is `not_started`, the reads before it succeeded but slowly: the database as a whole is slow (§8, and step 3). `[health-check] alerts held` lists the held alert names.
