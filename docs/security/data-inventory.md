...
| Recent Activity vocabulary receipt | `idempotency_receipts.payload` (`activity_vocabulary_reconciliation`) | Content-minimal evidence binding one reviewed source-code group, exact fingerprint, canonical destination, actor, and support reference; no projection row IDs or payloads | 30 days                                                                                                                       | Bounded shared receipt retention                                                                          |
...
| Destination action session ID    | `idempotency_receipts.payload` (`guest_destination_action`) | First-action integrity                                                                             | Random signed-session UUID                                        | Live checks deny after signed-session expiry; shared receipt retained for 30 days                               |
...
| Shared external-id receipts      | `idempotency_receipts`                                                                             | Deduplication for webhooks and other externally keyed operations                                                                                                                                                   | 30 days                                                                                                                                 |
...
| Organization lifecycle evidence  | `organization_lifecycle_events`, `organization_export_retrieval_issuances`                         | Append-only content-free events for command retries, context phases, Property erase, privacy transitions, and backup hold releases; digest-only history permanently retires each issued export retrieval authority | Retention not yet activated; must cover the recovery/retry horizon and approved legal evidence policy                                   |
